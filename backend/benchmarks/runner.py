...
from backend.agent import supervisor

client = Client()

def redmine_benchmark_target(inputs: dict) -> dict:
    """Target funtion for LangSmith - invoke agent benchmark case."""
    messages = inputs["messages"]
    result = supervisor.invoke({"messages": messages})
    return {"messages": result["messages"]}

benchmark_case = [
    {"inputs": {"messages": [HumanMessage(content="Get open issues in project ai-chatbot-platform")]}},
