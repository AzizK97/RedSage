...
class ThreadRepository:
    ...
    def get_project_identifier(self, thread_id: str) -> str | None:
        with self.db.cursor() as cur:
            cur.execute(
                "SELECT project_identifier FROM thread_owners WHERE thread_id = %s",
                (thread_id,),
            )
            row = cur.fetchone()
        return row[0] if row and row[0] is not None else None

    def set_project_identifier(self, thread_id: str, project_identifier: str | None) -> None:
        now = datetime.now(timezone.utc)
        with self.db.cursor() as cur:
            cur.execute(
                "UPDATE thread_owners SET project_identifier = %s, updated_at = %s WHERE thread_id = %s",
                (project_identifier, now, thread_id),
            )
        self.db.commit()
