    ...
    <div class="max-w-7xl mx-auto px-6">
      ...
      <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <AnimatedSection v-for="(agent, i) in agents" :key="agent.name" :delay="i * 100">
          <div class="group glass-card p-6 h-full hover:border-sage-500/25 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-sage-900/10 shimmer-bg">
            ...
            <h3 class="text-lg font-bold text-white mb-2">{{ agent.name }}</h3>
            <p class="text-sm text-surface-400 mb-5 leading-relaxed">{{ agent.description }}</p>
            <div class="space-y-2">
              <div v-for="ex in agent.examples" :key="ex" class="flex items-start gap-2 text-xs text-surface-500">
