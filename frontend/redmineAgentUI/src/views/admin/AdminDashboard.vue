...
import { listPmCandidates, pingApi, setPmAccess, syncProjectManagers, type PmCandidate } from "../../api/admin";
...
async function loadPmRows() {
  ...
  const response = await listPmCandidates(props.token.trim());
...
async function refreshFromRedmine() {
  ...
  try {
    const sync = await syncProjectManagers(props.token.trim());
...
async function toggleAccess(pm: PmCandidate, enabled: boolean) {
  ...
  try {
    const result = await setPmAccess(props.token.trim(), {
      redmine_user_id: pm.redmine_user_id,
      enabled,
    });

    rows.value = rows.value.map((row) =>
      row.redmine_user_id === pm.redmine_user_id ? { ...row, enabled } : row,
    );

    if (enabled && result.generated_password) {
      successMessage.value = `${pm.full_name} enabled. Credentials: ${result.email} / ${result.generated_password}`;
    } else if (enabled) {
      successMessage.value = `${pm.full_name} enabled. Account already existed; no new password was generated.`;
    } else {
      successMessage.value = `${pm.full_name} is now ${enabled ? "enabled" : "disabled"}.`;
    }
  } catch (error) {
