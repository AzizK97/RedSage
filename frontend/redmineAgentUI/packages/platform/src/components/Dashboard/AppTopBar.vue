...
let lastSeenCreatedAt: number | null = null;
...
async function loadNotifications() {
  ...
  try {
    ...
    const latest = response.items[0]?.created_at ?? 0;
...
<template>
  <header class="h-16 border-b border-surface-800 bg-surface-950 px-6 flex items-center justify-between gap-4 sticky top-0 z-30" style="z-index: 99999;">
    <div class="flex-1 max-w-2xl relative">
      <div class="relative group">
        <Search
          :size="16"
          class="absolute left-3.5 top-1/2 -translate-y-1/2 text-surface-500
             group-focus-within:text-sage-400 transition-colors z-index-1000"
        />
        <input
          v-model="query"
          @input="onSearchInput"
          ...
          class="w-full bg-surface-900 border border-surface-800 text-surface-200 text-sm
             rounded-xl py-2 pl-10 pr-4 outline-none focus:border-sage-500/40
             focus:ring-4 focus:ring-sage-500/5 transition-all shadow-inner"
        />
      </div>

      <div
        v-if="showResults"
        class="absolute top-[calc(100%+8px)] left-0 w-full bg-surface-900 border
           border-surface-800 rounded-2xl shadow-2xl shadow-black/50
           overflow-hidden flex flex-col"
      >
        <div v-if="isSearching" class="p-6 flex items-center justify-center gap-2 text-surface-500">
          <div class="w-4 h-4 border-2 border-sage-500/30 border-t-sage-500 rounded-full animate-spin" />
        ...
        <ul v-else-if="results.length" class="divide-y divide-surface-800/60">
            ...
            class="flex flex-col gap-1 px-4 py-3 hover:bg-surface-800 cursor-pointer
               transition-colors group"
            >
            <div class="flex items-center justify-between gap-3">
              <span class="text-sm font-medium text-surface-200 group-hover:text-sage-300
    ...
    <div class="flex items-center gap-3">
      ...
      <div class="relative">
        ...
        <div v-if="notificationsOpen" class="absolute top-[calc(100%+12px)] right-0 w-[420px] max-h-[80vh] bg-surface-900 border border-surface-800 rounded-2xl shadow-2xl shadow-black/50 overflow-hidden flex flex-col backdrop-blur-xl animate-scale-in origin-top-right" style="z-index: 99999;">
                ...
                <div class="flex gap-3">
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="flex items-start justify-between gap-2 mb-1">
                      ...
                      <span class="text-[10px] font-bold text-surface-500 whitespace-nowrap uppercase tracking-wider">{{ timeAgo(item.created_at ?? 0) }}</span>
                    </div>
